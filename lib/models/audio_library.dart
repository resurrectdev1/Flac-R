...
class AudioLibrary extends ChangeNotifier {
  ...
  Map<String, List<AudioFile>> get albums =>
      _albumCache ??= _buildGroup((f) => f.album);
  Map<String, List<AudioFile>> get artists =>
      _artistCache ??= _buildGroup((f) => f.artist);
...
class AudioScanner {
  ...
  static Future<AudioFile?> _readOne(File entity) async {
    ...
    try {
      ...
      final discNumber =
          _parseSlashInt(tag?.discNumber?.toString()) ?? tag?.discNumber;
  ...
  static Future<int?> _fallbackTrackNumber(
    String path,
    String lowerPath,
  ) async {
